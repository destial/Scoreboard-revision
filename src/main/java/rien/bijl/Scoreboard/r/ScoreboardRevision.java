...
import rien.bijl.Scoreboard.r.Plugin.Listeners.JoinLeaveListener;
...
public class ScoreboardRevision {
    ...
    private void setupBasics() {
        ...
        Session.getSession().defaultBoard = new ConfigBoard(defaultBoard);
        ...
        Session.getSession().defaultBoard.runTaskTimerAsynchronously(plugin, 1L, 1L);
        Session.getSession().worldManager = new WorldManager();
        Session.getSession().worldManager.runTaskTimer(plugin, 1L, 20L);
        Objects.requireNonNull(plugin.getCommand("sb")).setExecutor(new Manager());
    }
